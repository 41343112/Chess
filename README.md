...
1. **啟動遊戲 - 選擇模式**
   ...
   - 選擇遊戲模式：**玩家對戰** 或 **與電腦對戰** ⭐
   - 如選擇電腦對戰，可設定：
     - 難度：使用滑桿調整技能等級 0-20（使用 Stockfish 引擎）
     - 您的顏色：白方（先手）或黑方（後手）
   - 設定時間控制（可選）
   - 點擊「開始遊戲」
...
### 遊戲模式
- ✅ **玩家對戰模式** - 兩位玩家輪流對戰 ⭐
- ✅ **電腦對戰模式** - 使用世界級 Stockfish 引擎對戰 ⭐ 最新功能
  - 技能等級 0-20 可調整滑桿
  - 等級 0-5：初學者（約 1000-1400 Elo）
  - 等級 6-10：中等（約 1400-1800 Elo）
  - 等級 11-15：進階（約 1800-2200 Elo）
  - 等級 16-20：專家（2200+ Elo，等級 20 可達 3000+）
...
### Stockfish 引擎整合 ⭐ 最新
本遊戲使用世界頂級的 **Stockfish 16** 國際象棋引擎：
- **UCI 協議通訊**：透過標準 UCI 協議與引擎互動
- **技能等級 0-20**：完全可調的難度範圍
  - 自動 Elo 評級限制（低等級）
  - 可調思考時間（100ms 到 1100ms）
- **跨平台支援**：Windows、Linux、macOS
- **FEN 轉換**：自動將棋盤轉換為標準 FEN 格式
- **即時響應**：快速計算最佳移動

詳細技術實作請參閱 [COMPUTER_OPPONENT.md](docs/features/COMPUTER_OPPONENT.md)

## 未來改進

未來版本可能的新增功能：
- ✅ ~~完整的悔棋/重做功能~~ **已完成！**
- ✅ ~~AI 對手~~ **已完成！使用 Stockfish 引擎，支援 0-20 技能等級**
- 重做功能（Redo）
- 使用代數記譜法的移動歷史顯示
- 計時遊戲的計時器/時鐘
- 儲存/載入遊戲狀態
...
- 開局庫和遊戲分析
- 線上多人對戰
- Stockfish 進階功能（引擎評估顯示、多執行緒、Syzygy 殘局表格）
